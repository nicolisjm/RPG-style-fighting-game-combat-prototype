...
@export var skill_tag: int = CombatState.SkillTag.ENDER
@export var startup_duration: float = 0.3
@export var active_start: float = 0.35
@export var active_end: float = 0.5
@export var total_duration: float = 1.0
## Time until the player can move again.
@export var lock_duration: float = 0.7
@export var damage: float = 25.0
@export var hitstun_duration: float = 0.6
@export var knockback_force: float = 5.0
## Tilts the slash visual toward the camera (degrees)
@export var camera_tilt: float = 0
## Upward launch force (enables juggle starter)
@export var launch_force: float = 7.0
## Vertical force applied when the enemy is already airborne
@export var juggle_force: float = 6.0
## Brief freeze on hit — longer than normal for heavy impact feel
@export var hitstop_duration: float = 0.12

var _elapsed: float = 0.0
var _hit_bodies: Array[Node3D] = []
var _shapes_enabled_once: bool = false
var _visual_shown: bool = false
var _lock_emitted: bool = false
var _visual: Area3D = null

const VISUAL_SCENE := preload("res://scenes/attacks/sword/heavy_slash.tscn")
...
func _ready() -> void:
	...
	if not visual_node is Area3D:
		push_error("[HeavySlashSkill] Visual scene root must be Area3D")
		return
...
func _process(delta: float) -> void:
	...
	if not _visual_shown and _elapsed >= startup_duration:
		_visual_shown = true
...
func _on_body_entered(body: Node3D) -> void:
	...
	if body.has_method("hit"):
		body.hit(damage, self, knockback_force, hitstun_duration, launch_force, juggle_force)
